...
},
{
  ...
  Etat_main_levée: "En cours",
  Date_réception_PV_définitif: null,
  Observation:"Reste la validation du directeur de l'IRT + PAM (PV DE VALIDATION). Garantie 10 mois, réception déf 30 jrs après la garantie.",
...
const cautionSlice = createSlice({
  ...
  reducers: {
    getOneCaution: (state,action) => {
      state.caution=state.cautions.filter(caution=>
        caution.id===action.payload.id
      )[0]
    },
    deleteCaution: (state,action) => {
      state.cautions=state.cautions.filter(caution=>caution.id!==action.payload.id)
    ...
    updateCaution:(state,action)=>{
      state.cautions.map(caution=>{
        if(caution.id===action.payload.id){
          ...
          caution.Date_réception_PV_définitif=action.payload.caution.Date_réception_PV_définitif 
        }
      })
    },
